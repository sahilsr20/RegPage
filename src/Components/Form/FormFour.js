...
import WhichOneRadio from "./WhichOneRadio";
  ...
  const submitFunction = async () => {
    ...
    for (let i = 0; i < 4; i++) {
      if (whichOneRef.current && whichOneRef.current[i].checked === true)
        whichOne = whichOne + " " + whichOneRef.current[i].value;
    }
    ...
    const url = "https://fedkiit.com/fedReg/saveData";
      ...
      <div className={FormCss.contDiv}>
        ...
        <div className={FormCss.questionsDiv}>
          {stage === "yes" && (
            <WhichOneRadio
              question="Which One?"
              value={["Stonkaholic", "Pitchers", "Omega", "Other"]}
              ref={whichOneRef}
            />
          )}
          {stage !== "eventBefore" && (
